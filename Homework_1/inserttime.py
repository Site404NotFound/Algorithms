...
**
** Description: Homework #1 (Problem 5)
** Due: Sunday, April 08, 2018
**
** Filename: inserttime.py
**
** Objectives:
...
if __name__ == "__main__":
    ...
    for count in range(0, 13):                                                  # Run the sort several times with different array sizes
        starttime = time.time()                                                 # Collect the start time at the initial execution of the program
        if count == 1:
            n = 2000                                                            # Set Array size to 2000 for the second insertion sort run
        if count == 2:
            n = 5000                                                            # Set Array size to 5000 for the third insertion sort run
        if count == 3:
            n = 10000                                                           # Set Array size to 10000 for the forth insertion sort run
        unsortedArray = []                                                      # Establish a new array to store the random integers
        for x in range (n):                                                     # Run through a loop for the desired number of random elements
            unsortedArray.append(random.randint (0, 10000))                     # Append the random elements to the unsorted Array
        insertionSort(unsortedArray, n)                                         # Send the array to the sort function to be sorted
